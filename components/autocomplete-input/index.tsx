    ...
    >
      <>
        <Combobox.Input
          ...
          onClick={() => {
            !disabled && buttonRef?.current?.click()
          }}
          onBlur={() => {
            setIsActive(false)
          }}
          displayValue={(
            options: typeof multiple extends true
              ? TOptions[number][]
              : TOptions[number]
          ) => {
            if (Array.isArray(options)) {
              const labels = options?.map((o: AutocompleteOption) => o.label)
              const labelsSet = new Set(labels)
              const uniqLabels = Array.from(labelsSet).join(', ')
              if (uniqLabels.length > 16) {
                return `${uniqLabels.substring(0, 16).trimEnd()} ...`
              }
              return uniqLabels
            }
